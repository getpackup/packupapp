...
const TripCard = ({ trip, showCountdown, showRemaining, isPending, refetch }: TripCardProps) => {
  ...
  const sendTripMemberStatusMessage = (status: TripMemberStatus) => {
    if (!user?.uid || !user?.username) return

    const newMessage: Omit<ChatMessage, 'id'> = {
      // id: '', // will get generated by the Firestore database
      userId: user?.uid ?? '',
      content: `@${user.username} has ${status.toLowerCase()} the trip invitation.`,
      ...
      updateDocument(
        { id: trip.tripId, data: payload },
        {
          onSuccess: () => {
            ...
            //   status,
            // })
            sendTripMemberStatusMessage(status)
